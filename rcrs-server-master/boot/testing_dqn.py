...
def play(episodes, is_render, is_testing, checkpoint_interval,
    ...
    statistics_header.extend(["eps_greedy_{}".format(i) for i in range(n_agents)])
    statistics_header.extend(["collisions_{}".format(i) for i in range(n_agents)])
    ...
    for episode in range(args.episodes):
        states = env.reset()
        episode_losses = np.zeros(n_agents)
        ...
        while True:
            ...
            if any(done):
                ...
                statistic.extend([episode_rewards[i] for i in range(n_agents)])
                statistic.extend([episode_losses[i] for i in range(n_agents)])
                statistic.extend([dqns[i].eps_greedy for i in range(n_agents)])
...
if __name__ == '__main__':
    ...
    if args.epsilon_greedy is not None:
        if len(args.epsilon_greedy) == n_agents:
            epsilon_greedy = args.epsilon_greedy
        else:
            raise ValueError("Must have enough epsilon_greedy for all agents")
    else:
        epsilon_greedy = [0.5 for i in range(n_agents)]
    ...
    # init DQNs
    n_actions = [env.action_space[i].n for i in range(n_agents)]
    state_sizes = [env.observation_space[i].shape[0] for i in range(n_agents)]
    memories = [Memory(args.memory_size) for i in range(n_agents)]
